import {applyResponsive, getResponsiveParameter} from "./applyResponsive";

const constantFive = () => 5;
const responsiveConstantFive = applyResponsive(constantFive);

const identity = (x) => x;
const responsiveIdentity = applyResponsive(identity);

const sum = (a, b) => a + b;
const responsiveSum = applyResponsive(sum);

const varArgsSum = (...args) => args.reduce((sum, current) => sum + current, 0);
const responsiveVarArgsSum = applyResponsive(varArgsSum);
...
const responsiveGetContainerWidth = applyResponsive(getContainerWidth);
...
const responsiveMaxValueKey = applyResponsive(maxValueKey);
